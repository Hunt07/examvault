...
import { Award, CheckCircle, X, Info } from 'lucide-react';
...
const ToastNotification: React.FC<ToastNotificationProps> = ({ message, points, type = 'success', onClose }) => {
  ...
  const bgColor = type === 'error' ? 'bg-red-50 dark:bg-red-900/20' : (type === 'info' ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-white dark:bg-zinc-800');
  const borderColor = type === 'error' ? 'border-red-500' : (type === 'info' ? 'border-blue-500' : 'border-primary-500');
      ...
      <div className={`${bgColor} border-l-4 ${borderColor} shadow-lg rounded-lg p-4 flex items-center gap-4 pr-12 relative min-w-[300px]`}>
        ...
        <div>
            <h4 className={`font-bold ${textColor} text-base`}>
                {points ? (points > 0 ? `+${points} Points Earned!` : `${points} Points`) : (type === 'error' ? 'Error' : (type === 'info' ? 'Update' : 'Success'))}
            </h4>

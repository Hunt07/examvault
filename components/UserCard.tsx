...
const UserCard: React.FC<UserCardProps> = ({ user, onSelect }) => {
  ...
  return (
    ...
    >
      <Avatar
        className="w-24 h-24 rounded-full border-4 border-slate-200 dark:border-zinc-700 group-hover:border-primary-400 dark:group-hover:border-primary-500 transition-colors text-4xl"
        src={user.avatarUrl}
        name={user.name}
      />

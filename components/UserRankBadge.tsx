...
const UserRankBadge: React.FC<UserRankBadgeProps> = ({ rank, size = 24 }) => {
  ...
  // Generic Rank Badge for > 2
  return (
      <div 
        className="inline-flex items-center justify-center bg-slate-100 dark:bg-zinc-700 text-slate-600 dark:text-slate-300 font-bold rounded-full ml-2 shrink-0 border border-slate-200 dark:border-zinc-600 shadow-sm"
        style={{ width: size * 0.9, height: size * 0.9, fontSize: size * 0.4 }}
        title={`Rank #${rank + 1}`}
      >
          {rank + 1}
      </div>

...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, allResources, isCurrentUser }) => {
    ...
    const userForumPosts = useMemo(() => forumPosts.filter(p => p.author.id === user.id), [forumPosts, user.id]);
    const userOpenRequests = useMemo(() => isCurrentUser ? resourceRequests.filter(r => r.requester.id === user.id && r.status === ResourceRequestStatus.Open) : [], [resourceRequests, user.id, isCurrentUser]);

...
const ResourceDetailPage: React.FC<{ resource: Resource }> = ({ resource }) => {
  ...
  const prepareAIContent = async () => {
      ...
      return { text: aiText, base64, mimeType, source, metadataText: fullText };
  };
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const { text, base64, mimeType, source, metadataText } = await prepareAIContent();
      try {
        ...
        if (source === 'file') {
          const fallbackResult = await summarizeContent(metadataText, undefined, undefined);
  ...
  const handleGenerateStudySet = async (type: 'flashcards' | 'quiz') => {
    ...
    try {
      ...
      try {
        ...
        if (source === 'file') {
          const fallbackResult = await generateStudySet(metadataText, type);
          setStudySet(fallbackResult);
        } else {
          throw primaryError;
        }
      }

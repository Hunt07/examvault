...
const ResourceDetailPage: React.FC<{ resource: Resource }> = ({ resource }) => {
  ...
  const getMetadataContext = () => {
      return `Title: ${resource.title}\nCourse: ${resource.courseCode}\nType: ${resource.type}\nLecturer: ${resource.lecturer || 'N/A'}`;
  };

  const handleGenerateSummary = async () => {
    setIsSummarizing(true);
    setSummary('');
    const result = await summarizeContent(getMetadataContext(), resource.fileBase64, resource.mimeType, resource.extractedText);
    setSummary(result);
    setIsSummarizing(false);
  };
  ...
  const handleGenerateStudySet = async (type: 'flashcards' | 'quiz') => {
    ...
    const result = await generateStudySet(getMetadataContext(), type, resource.fileBase64, resource.mimeType, resource.extractedText);
    setStudySet(result);
    setIsGeneratingStudySet(false);
  };
  ...
  const renderPreviewContent = () => {
    ...
    if (isMock) {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                ...
                <p className="text-slate-600 mb-6 max-w-md">Mock resource preview using extracted text content.</p>
                <div className="w-full max-w-3xl bg-white rounded-lg border border-slate-200 p-6 text-left h-96 overflow-y-auto shadow-inner"><MarkdownRenderer content={resource.contentForAI} /></div>
            </div>
        );
    }
    ...
    if (isOfficeDoc) return <iframe src={`https://docs.google.com/gview?url=${encodeURIComponent(resource.fileUrl)}&embedded=true`} className="w-full h-full border-none" title="Office Preview" />;
        ...
        <div className="flex flex-col items-center justify-center h-full p-8 text-center">
            ...
            <div className="w-full max-w-3xl bg-white rounded-lg border border-slate-200 p-6 text-left h-96 overflow-y-auto shadow-inner relative flex flex-col">
                ...
                <div className="flex-grow overflow-y-auto">
                    ...
                    ) : contentToDisplay ? <MarkdownRenderer content={contentToDisplay} /> : (
                        <div className="flex flex-col items-center justify-center h-full text-slate-400 py-12">
                            <button onClick={handleGeneratePreview} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700 transition font-semibold text-sm shadow-sm"><BrainCircuit size={16} /> Generate AI Summary</button>
                        </div>
                    )}
                </div>
            </div>
  ...
  return (
    <div>
      <button onClick={() => setView('dashboard')} className="flex items-center gap-2 text-primary-600 dark:text-primary-400 font-semibold hover:text-primary-800 dark:hover:text-primary-300 transition mb-6"><ArrowLeft size={20} /> Back to all resources</button>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md transition-colors duration-300 border border-transparent dark:border-zinc-700">
            <div className="flex items-center gap-3 mb-4">
              <span className={`flex items-center gap-2 text-sm font-semibold px-3 py-1 rounded-full ${getBadgeStyle(resource.type)}`}>{getBadgeIcon(resource.type)}{resource.type}</span>
            ...
            <div className="mt-6 pt-6 border-t border-slate-200 dark:border-dark-border space-y-4">
                <div>
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => toggleCourseCodeSubscription(resource.courseCode)} className={`flex items-center gap-2 text-sm font-semibold px-3 py-2 rounded-lg transition ${isFollowingCourse ? 'bg-primary-600 text-white hover:bg-primary-700' : 'bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-zinc-700'}`}>{isFollowingCourse ? <BellOff size={16} /> : <Bell size={16} />}<span>{isFollowingCourse ? 'Unfollow Course' : 'Follow Course'}</span></button>
                        {resource.lecturer && (
                             <button onClick={() => toggleLecturerSubscription(resource.lecturer!)} className={`flex items-center gap-2 text-sm font-semibold px-3 py-2 rounded-lg transition ${isFollowingLecturer ? 'bg-primary-600 text-white hover:bg-primary-700' : 'bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-zinc-700'}`}>{isFollowingLecturer ? <BellOff size={16} /> : <Bell size={16} />}<span>{isFollowingLecturer ? 'Unfollow Lecturer' : 'Follow Lecturer'}</span></button>
                        )}
                    </div>
                </div>
                <div className="flex flex-col gap-3 !mt-6">
                    <button onClick={() => setIsPreviewOpen(true)} className="w-full flex items-center justify-center gap-2 font-bold py-3 px-4 rounded-lg transition-all duration-200 bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-zinc-700 hover:text-primary-700 dark:hover:text-primary-400 border border-slate-200 dark:border-zinc-700"><Eye size={18} /> Preview File</button>
                    <a href={resource.fileUrl} download={resource.fileName} onClick={handleDownloadClick} className={`w-full flex items-center justify-center gap-2 font-bold py-3 px-4 rounded-lg transition-all duration-200 ${isDownloading ? 'bg-primary-700 text-primary-100 cursor-wait' : 'bg-primary-600 text-white hover:bg-primary-700 hover:-translate-y-0.5 shadow-md hover:shadow-lg'}`}>{isDownloading ? <><Loader2 size={18} className="animate-spin" /> Downloading...</> : <><Download size={18} /> Download</>}</a>
                </div>
                {hasReported ? (
                     <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800 text-center flex flex-col items-center gap-2"><CheckCircle className="text-green-600 dark:text-green-400" size={24}/><div><h4 className="font-bold text-green-800 dark:text-green-200">Report Submitted</h4><p className="text-sm text-green-700 dark:text-green-300 mt-1">Thank you. Our moderators will review this shortly.</p></div></div>
                ) : !isReporting ? (
                    <button onClick={() => setIsReporting(true)} className="w-full flex items-center justify-center gap-2 text-slate-500 dark:text-slate-400 font-semibold py-2 px-4 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 hover:text-slate-700 dark:hover:text-slate-200 transition text-sm"><Flag size={16} className="text-red-500"/> Report this resource</button>
                ) : (
                    <div className="p-4 bg-slate-50 dark:bg-zinc-800/50 rounded-lg border border-slate-200 dark:border-zinc-700"><h4 className="font-bold text-slate-700 dark:text-slate-200 mb-2">Report Resource</h4><textarea value={reportReason} onChange={(e) => setReportReason(e.target.value)} placeholder="Reason for report..." className="w-full bg-white dark:bg-zinc-900 text-slate-900 dark:text-white p-4 border rounded-lg focus:ring-primary-500 outline-none" rows={3} autoFocus /><div className="flex justify-end gap-2 mt-4"><button onClick={() => setIsReporting(false)} className="bg-slate-200 dark:bg-zinc-700 text-slate-700 dark:text-slate-200 font-semibold py-2 px-4 rounded-lg hover:bg-slate-300 transition">Cancel</button><button onClick={handleSubmitReport} className="bg-red-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-red-700 transition">Submit Report</button></div></div>
                )}
            </div>
          </div>
          <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md mt-8 transition-colors duration-300 border border-transparent dark:border-zinc-700">
            ...
            {!summary && !isSummarizing && (
              <div className="border-2 border-dashed border-slate-300 dark:border-zinc-700 rounded-lg p-6 text-center">
                  <BrainCircuit className="mx-auto h-12 w-12 text-slate-400 dark:text-slate-500" /><button onClick={handleGenerateSummary} className="mt-4 inline-flex items-center gap-2 bg-primary-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-primary-700 transition"><BrainCircuit size={18} /> Generate with Gemini</button>
              </div>
            )}
            {isSummarizing && <div className="border border-slate-200 dark:border-zinc-700 rounded-lg p-6 text-center"><Loader2 className="mx-auto h-12 w-12 text-primary-500 animate-spin" /><p className="mt-4 text-slate-600 dark:text-slate-300 font-medium">Gemini is analyzing content...</p></div>}
          ...
          <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md mt-8 transition-colors duration-300 border border-transparent dark:border-zinc-700">
            ...
            {isGeneratingStudySet ? <div className="border border-slate-200 dark:border-zinc-700 rounded-lg p-6 text-center"><Loader2 className="mx-auto h-12 w-12 text-primary-500 animate-spin" /><p className="mt-4 text-slate-600 dark:text-slate-300 font-medium">Generating {studySetType}...</p></div> : !studySet ? (
                <div className="border-2 border-dashed border-slate-300 dark:border-zinc-700 rounded-lg p-6 text-center"><div className="flex justify-center gap-4"><button onClick={() => handleGenerateStudySet('flashcards')} className="flex-1 flex flex-col items-center gap-2 bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-slate-200 font-bold py-4 rounded-lg hover:bg-slate-100 transition border dark:border-zinc-700"><BookCopy size={24} /> Flashcards</button><button onClick={() => handleGenerateStudySet('quiz')} className="flex-1 flex flex-col items-center gap-2 bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-slate-200 font-bold py-4 rounded-lg hover:bg-slate-100 transition border dark:border-zinc-700"><HelpCircle size={24} /> Practice Quiz</button></div></div>
            ) : <div className="bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-zinc-700 rounded-lg p-4 sm:p-6">{studySetType === 'flashcards' ? <FlashcardViewer flashcards={studySet as Flashcard[]} onReset={resetStudySet} /> : <QuizComponent questions={studySet as QuizQuestion[]} onReset={resetStudySet} />}</div>}
          </div>
          <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md mt-8 transition-colors duration-300 border border-transparent dark:border-zinc-700">
            ...
            <form onSubmit={handlePostComment} className="flex gap-4 items-start pb-6 mb-6 border-b dark:border-zinc-700"><Avatar src={user?.avatarUrl} alt={user?.name} className="w-10 h-10 shrink-0" /><div className="w-full"><MarkdownToolbar textareaRef={commentTextareaRef} onValueChange={setNewComment} value={newComment} /><textarea ref={commentTextareaRef} value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Add a comment..." className="w-full bg-slate-100 dark:bg-zinc-800 dark:text-white p-4 border dark:border-zinc-700 rounded-b-lg focus:ring-primary-500 outline-none" rows={3} /><div className="flex justify-end mt-2"><button type="submit" className="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-700 transition">Post</button></div></div></form>
        ...
        <div className="lg:col-span-1">
            <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md lg:sticky top-24 transition-colors duration-300 border border-transparent dark:border-zinc-700">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={handleUpvoteClick} className={`flex items-center gap-2 p-3 rounded-lg transition font-medium ${isUpvoted ? 'bg-green-600 text-white' : 'bg-green-100 dark:bg-green-900/30 text-green-700'}`}><ThumbsUp size={18} /> {resource.upvotes > 0 && <span>{resource.upvotes}</span>}</button>
                    <button onClick={handleDownvoteClick} className={`flex items-center gap-2 p-3 rounded-lg transition font-medium ${isDownvoted ? 'bg-red-600 text-white' : 'bg-red-100 dark:bg-red-900/30 text-red-700'}`}><ThumbsDown size={18} /> {resource.downvotes > 0 && <span>{resource.downvotes}</span>}</button>
                    <button onClick={() => toggleSaveResource(resource.id)} className={`p-3 rounded-lg transition ${isSaved ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 dark:bg-zinc-800 text-slate-500'}`}>{isSaved ? <BookmarkCheck size={18} /> : <Bookmark size={18} />}</button>
                    <button onClick={() => setIsShareModalOpen(true)} className="p-3 rounded-lg bg-slate-100 dark:bg-zinc-800 text-slate-500 hover:bg-slate-200 transition"><Share2 size={18} /></button>
                    {isAuthor && <button onClick={() => setIsDeleteConfirmOpen(true)} className="p-3 rounded-lg bg-slate-100 dark:bg-zinc-800 text-red-500 hover:bg-red-100 transition"><Trash2 size={18} /></button>}
                </div>
                <div className="mt-6 pt-6 border-t border-slate-200 dark:border-dark-border"><p className="text-sm font-semibold text-slate-800 dark:text-white mb-3">Uploaded by</p><button onClick={() => setView('publicProfile', resource.author.id)} className="flex items-center gap-3 w-full text-left hover:bg-slate-50 dark:hover:bg-zinc-800 p-2 rounded-lg transition"><Avatar src={resource.author.avatarUrl} alt={resource.author.name} className="w-12 h-12" /><div><div className="flex items-center"><p className="font-bold text-slate-900 dark:text-slate-100">{resource.author.name}</p><UserRankBadge rank={authorRank} /></div><p className="text-xs text-slate-500 dark:text-slate-400">{resource.author.course} • Joined {new Date(resource.author.joinDate).toLocaleDateString()}</p></div></button></div>
            </div>
        </div>
      </div>
      {isDeleteConfirmOpen && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in"><div className="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-xl max-w-sm w-full border dark:border-zinc-700 text-center"><div className="p-3 bg-red-100 dark:bg-red-900/30 rounded-full text-red-600 dark:text-red-400 mb-4 mx-auto w-fit"><Trash2 size={32} /></div><h3 className="text-xl font-bold dark:text-white mb-2">Delete Resource?</h3><p className="text-slate-500 mb-6 text-sm">Are you sure? This cannot be undone.</p><div className="flex gap-3"><button onClick={() => setIsDeleteConfirmOpen(false)} className="flex-1 py-2.5 bg-slate-100 dark:bg-zinc-700 text-slate-700 dark:text-slate-200 rounded-lg">Cancel</button><button onClick={confirmDelete} className="flex-1 py-2.5 bg-red-600 text-white rounded-lg">Delete</button></div></div></div>
      )}
      {isPreviewOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200"><div className="bg-white dark:bg-dark-surface rounded-xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col border dark:border-zinc-700"><div className="p-4 border-b dark:border-zinc-700 flex justify-between items-center bg-slate-50 dark:bg-zinc-800/50 rounded-t-xl"><div><h3 className="font-bold dark:text-white truncate">{resource.title}</h3><p className="text-xs text-slate-500">{resource.fileName}</p></div><button onClick={() => setIsPreviewOpen(false)} className="p-2 rounded-full hover:bg-red-100 text-slate-500 transition"><X size={24} /></button></div><div className="flex-grow bg-slate-200 dark:bg-zinc-900 overflow-hidden flex items-center justify-center rounded-b-xl relative">{renderPreviewContent()}</div></div></div>
    )}

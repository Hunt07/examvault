...
import { ArrowLeft, ArrowRight, ThumbsUp, ThumbsDown, MessageSquare, Download, Loader2, FileText, Notebook, ClipboardList, Archive, Flag, CheckCircle, MessageCircle, Eye, X, AlertCircle, Bookmark, BookmarkCheck, Share2, Trash2, Paperclip, Image as ImageIcon, BrainCircuit, BookCopy, HelpCircle, FileCheck } from 'lucide-react';
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="font-bold text-slate-900 dark:text-slate-100 hover:text-primary-600 dark:hover:text-primary-400 text-sm hover:underline"
                >
                  {comment.author.name}
                </button>
              ...
              {comment.attachment.type === 'image' ? (
                <button onClick={() => onPreview(comment.attachment!)} className="block cursor-zoom-in">
                  <img
                    ...
                    alt="Attachment"
                    className="max-h-40 rounded-lg border border-slate-200 dark:border-zinc-700 hover:opacity-90 transition"
                  />
                </button>
              ) : (
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition">
                    ...
                    <div className="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-xl max-w-sm w-full border dark:border-zinc-700">
                      ...
                      <div className="flex gap-3 w-full">
                        <button
                          ...
                          className="flex-1 py-2 bg-slate-100 dark:bg-zinc-700 text-slate-700 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-200 dark:hover:bg-zinc-600 transition"
                        >
                          Cancel
                        </button>
                        <button onClick={handleDelete} className="flex-1 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition">
                          Delete
                        </button>
                      </div>
                    </div>
...
const ResourceDetailPage: React.FC<{ resource: Resource }> = ({ resource }) => {
  const { user, userRanks, setView, handleVote, addCommentToResource, savedResourceIds, toggleSaveResource, resources, deleteResource, scrollTargetId, setScrollTargetId } =
  ...
  const prepareAIContent = async () => {
    // @ts-ignore (if your type doesn't include it yet)
    const extracted = (resource.contentForAI || "").trim();

    const fileData = await resolveFileBase64();

    if (fileData?.base64 && fileData?.mimeType) {
      return { mode: "inlineData" as const, text: "", base64: fileData.base64, mimeType: fileData.mimeType };
    }

    if (extracted.length > 50) {
      return { mode: "text" as const, text: extracted, base64: undefined, mimeType: undefined };
    }

    return { mode: "none" as const, text: "", base64: undefined, mimeType: undefined };
  };
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const payload = await prepareAIContent();

      if (payload.mode === "inlineData") {
        const result = await summarizeContent("", payload.base64, payload.mimeType);
        setSummary(result);
      ...
      if (payload.mode === "text") {
        const result = await summarizeContent(payload.text);
        setSummary(result);
        setSummarySource('file');
        return;
      }

      setAiError("No usable file content available (cannot fetch bytes and no extracted text saved).");
    } catch (e: any) {
  ...
  const handleGenerateStudySet = async (type: 'flashcards' | 'quiz') => {
    ...
    try {
      ...
      if (payload.mode === "inlineData") {
        const result = await generateStudySet("", type, payload.base64, payload.mimeType);
        setStudySet(result);
        return;
      }

      if (payload.mode === "text") {
        const result = await generateStudySet(payload.text, type);
        setStudySet(result);
        return;
      }
      ...
      setAiError(e?.message || "Failed to generate study set.");
      setStudySet([]);
    } finally {
      setIsGeneratingStudySet(false);
    }
              ...
              <div className="flex flex-col gap-3 !mt-6">
                ...
                <a
                  ...
                  className={`w-full flex items-center justify-center gap-2 font-bold py-3 px-4 rounded-lg transition-all duration-200 ${
                    isDownloading ? 'bg-primary-700 text-primary-100 cursor-wait' : 'bg-primary-600 text-white hover:bg-primary-700 hover:-translate-y-0.5 shadow-md hover:shadow-lg'
                  }`}
                >
                  {isDownloading ? (
                    <>
          ...
          <div className="bg-white dark:bg-dark-surface p-4 sm:p-6 rounded-xl shadow-md mt-8 transition-colors duration-300 border border-transparent dark:border-zinc-700">
            ...
            {isSummarizing && (
              <div className="border border-slate-200 dark:border-zinc-700 rounded-lg p-8 text-center bg-slate-50 dark:bg-zinc-800/30">
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Extracting content and generating insights. This may take a moment.</p>
              </div>
            )}
            ...
            {summary && (
              <div className="animate-fade-in">
                {summarySource && (
                  <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-300 dark:border-green-800">
                    <FileCheck size={14} /> Based on File Content
                  </div>
                )}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-3 rounded-lg transition font-medium ${
                  isSaved ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-zinc-700'
                }`}
              >

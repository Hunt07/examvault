...
interface StudySetParams extends AIServiceParams {
...
- **Potential Exam Questions:** 3 sample questions that could be asked based on this material.`;
    ...
    if (extractedText) {
        ...
        if (isWord) {
            ...
            if (!text || text.length < 50) return "⚠️ **Insufficient Text Content** for Word document.";
            parts.push({ text: `Document Metadata:\n${content}\n\nDocument Content:\n${text}` });
        } else if (isPowerPoint) {
            const text = await extractTextFromPptx(fileBase64);
            if (!text || text.length < 50) return "⚠️ **Insufficient Text Content** for presentation.";
            parts.push({ text: `Presentation Metadata:\n${content}\n\nPresentation Slides & Notes:\n${text}` });
        } else if (isPDF || isImage) {
            parts.push({ text: `Analyze document. Metadata: ${content}` });
            const cleanBase64 = fileBase64.replace(/^data:.+;base64,/, '');
        ...
        parts.push({ text: `Material to analyze:\n---\n${content}\n---` });
    }

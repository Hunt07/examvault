...
export const summarizeContent = async (
  ...
  try {
    ...
    if (extractedText) {
        // Use pre-extracted text (bypasses CORS)
        parts.push({ text: `Analyze this document. Context: ${metadata}\n\nContent:\n${extractedText}` });
    } else if (fileBase64 && mimeType) {
        // Use pre-loaded base64 (bypasses CORS)
        parts.push({ text: `Analyze this material. Context: ${metadata}` });
